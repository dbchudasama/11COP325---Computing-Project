...
unsigned int prim_poly[4] = {1, 0, 1, 1}; // Initialising primitive polynomial

GaloisField gf(3, prim_poly); // Creating Galois Field, where the integer value 
							  // represents the exponent n, i.e. 2^n. This value
							 // has to be changed according to the power

GaloisFieldElement alpha(&gf, 2); //Galois Field Element alpha...change value accordingly 
GaloisFieldElement beta(&gf, 4); // Galois Field Element beta...change value accordingly

unsigned int o = (2^3)-1; // Exponent power corresponds to value assigned above
...
int ans;
    ...
    cout<<"\n \n Implementation of the Exhaustive Search/Brute force method for" 
		"\n computing the unique integer x for the Discrete Logarithm Problem in" 
		"\n the finite field GF(2^n), where n is the value of the field exponent," 
		"\n alpha and beta are Galois Field Elements and prim_poly is the primitive" 
		"\n polynomial of the field. ";

    ans = ex_search(alpha, beta, 7); // Calling the function Exhaustive Search
